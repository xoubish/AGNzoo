...
def get_TDE_sample(coords, labels, *, object_ids, verbose=1):
...
def get_paper_sample(coords, labels, *, paper_link, label, verbose=1):
...
def clean_sample(coords_list, labels_list, *, consolidate_nearby_objects=True, verbose=1):
    ...
    if not consolidate_nearby_objects:
        sample_table['objectid'] = list(range(len(sample_table)))
        print(f'Object sample size: {len(sample_table)}')
