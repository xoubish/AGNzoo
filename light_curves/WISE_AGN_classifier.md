...
```{code-cell} ipython3
#check that this is doing what I want it to do
#make a histogram of the number of fluxes per object in W1 band
#groupby objectid then cound the number of fluxes
df_lc.groupby(["objectid"]).size().hist()


#can I figure out how many there are?
...
```{code-cell} ipython3
#I tried to convert to datetime first and 
#then use datetime facilities to make the evenly spaced array
#using 
#from datetime import timedelta
#delta = timedelta(days = 30)
#thirtyday_timestamps = []
#date_x = df_lc.datetime.min()
#while date_x < df_lc.datetime.max():
#  date_x = date_x + delta
#  thirtyday_timestamps.append(date_x) 

# but sklearn's knn can;t handle datetime timestamps, so this isn't a good solution
```
...
parquet_savename = 'output/df_lc_WISE_AGN_ML.parquet'
...
```{code-cell} ipython3
#how can they be not equally spaced?
X_train.groupby(["objectid"]).size().max(), X_train.groupby(["objectid"]).size().min()
```
...
```{code-cell} ipython3
#consider some other metrics to assess performance of the algorithm
#MCC is matthews_correlation coefficient
...
#homogeneity (which I think is purity)
...
```{code-cell} ipython3
MCC, completeness, homogeneity, f1
```
...
accscore_dict, completeness_dict, homogeneity_dict
```
